...
function replyTo(button) {
    ...
    if (replyText) {
        ...
        replyDiv.style.background = 'rgba(203, 163, 101, 0.1)';
        replyDiv.style.borderRadius = '8px';
        replyDiv.style.borderLeft = '3px solid #CBA365';
        replyDiv.innerHTML = `
            <p style="font-weight: 600; font-size: 0.85rem; color: #4A3B2C; margin-bottom: 5px;">Mempelai <i class="fas fa-heart" style="color: #CBA365; font-size: 0.7rem;"></i></p>
            <p style="font-size: 0.85rem; color: #8C6A4A;">${replyText}</p>
        `;
...
document.getElementById('wedding-form').addEventListener('submit', function (e) {
    ...
    newComment.style.border = '1px solid rgba(203, 163, 101, 0.2)';
    
    newComment.innerHTML = `
        <p style="font-weight: 600; font-size: 0.95rem; margin-bottom: 8px; color: #4A3B2C;">
            ${nama} <i class="fas fa-check-circle" style="color: ${hadir === 'Hadir' ? '#CBA365' : '#8C6A4A'}; font-size: 0.8rem; margin-left: 5px;"></i>
        </p>
        <p style="font-size: 0.9rem; color: #8C6A4A; margin-bottom: 10px; line-height: 1.5;">${ucapan}</p>
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <p style="font-size: 0.75rem; color: #A87C4F; font-family: 'Montserrat';"><i class="far fa-clock mr-1"></i> Baru saja</p>
            <button onclick="replyTo(this)" style="background: none; border: none; color: #CBA365; font-size: 0.75rem; cursor: pointer; font-weight: 600;">Balas</button>
        </div>
    `;
...
if(window.particlesJS) {
    particlesJS("particles-js", {
        "particles": {
            ...
            "color": { "value": ["#ffffff", "#F2E3C6", "#CBA365"] },
            "shape": { 
                "type": "image",
